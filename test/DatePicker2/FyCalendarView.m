...
- (void)setupNextAndLastMonthView {
    ...
    [leftBtn setTitle:@"前" forState:UIControlStateNormal];
    ...
    [rightBtn setTitle:@"后" forState:UIControlStateNormal];
...
-(void)logDate:(UIButton *)dayBtn
{
   ...
   NSDate *  senddate=[NSDate date];
    ...
    NSString *  locationMounthString=[dateformatter stringFromDate:senddate];
    
    NSInteger day = [[dayBtn titleForState:UIControlStateNormal] integerValue];
    
    NSLog(@"-----:%ld",(long)day);
    NSLog(@"(long)[self month:_date]---:%ld",(long)[self month:tempDate]);
    NSLog(@"[locationMounthString integerValue]---:%ld",(long)[locationMounthString integerValue]);
    if ((long)[self month:tempDate] > [locationMounthString integerValue]) {
        ...
        NSLog(@"stringFromDate---:%@",[self stringFromDate:self.date]);
        if (self.calendarBlock) {
            self.calendarBlock(day, [comp month], [comp year]);
        }
        
    }else if ((long)[self month:tempDate] == [locationMounthString integerValue]){
        [dateformatter setDateFormat:@"dd"];
        NSString *  locationString=[dateformatter stringFromDate:senddate];
        NSLog(@"locationString:%ld",(long)[locationString integerValue]);
        if (day > [locationString integerValue] ) {
            self.selectBtn.selected = NO;
            [self.selectBtn setBackgroundColor:[UIColor clearColor]];
            dayBtn.selected = YES;
            self.selectBtn = dayBtn;
